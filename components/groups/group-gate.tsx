...
// We don't need the router here anymore for the "Open" button
import { useRouter } from "next/navigation"

export default function GroupGate() {
  const { groups, selectGroup, createGroup, addMembers } = useAuth()
  const router = useRouter() // Keep router for other potential navigation
  ...
  const finishMembers = () => {
    if (createdGroupId) {
      // The addMembers function is a placeholder for now
      ...
      >
                  ...
                  {groups.map((g) => (
                      <li key={g.id} className="rounded-lg border border-white/10 bg-neutral-950 p-4">
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          {/* --- THIS IS THE FIX --- */}
                          {/* The onClick now ONLY calls selectGroup. The router.replace is removed. */}
                          <Button
                              size="sm"
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="border-white/10 bg-black/90 sm:max-w-md">
            <DialogHeader><DialogTitle className="text-foreground">Create a Group</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <label className="text-sm text-neutral-400">Group Name</label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Finance Team" className="border-white/10 bg-black/60" />
            </div>
            <DialogFooter className="gap-2">
              <DialogClose asChild><Button variant="outline" className="border-white/10 bg-transparent">Cancel</Button></DialogClose>
              <Button className="bg-teal-600 hover:bg-teal-500" onClick={onCreate}>Continue</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          ...
          <DialogContent className="border-white/10 bg-black/90 sm:max-w-md">
            <DialogHeader><DialogTitle className="text-foreground">Add members (optional)</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div>
                <label className="text-sm text-neutral-400">Invite by email</label>
                <div className="mt-2 flex items-center gap-2">
                  <Input value={emailInput} onChange={(e) => setEmailInput(e.target.value)} placeholder="name@company.com" className="flex-1 border-white/10 bg-black/60" />
                  <Button type="button" variant="outline" onClick={addEmail} className="border-teal-500/40 text-teal-300 hover:bg-teal-500/10 bg-transparent">Add</Button>
                </div>
              </div>
              {emails.length > 0 && (
                  <ul className="max-h-40 overflow-y-auto rounded border border-white/10 bg-neutral-950 p-2 text-sm">
                    {emails.map((e, i) => (
                        <li key={`${e}-${i}`} className="flex items-center justify-between rounded px-2 py-1">
                          ...
                          <Button variant="ghost" size="sm" className="text-neutral-400 hover:text-red-300" onClick={() => removeEmail(i)}>Remove</Button>
                        </li>
                    ))}
                  </ul>
              )}
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" className="border-white/10 bg-transparent" onClick={skipMembers}>Skip for now</Button>
              <Button className="bg-teal-600 hover:bg-teal-500" onClick={finishMembers}>Save & Continue</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </section>
